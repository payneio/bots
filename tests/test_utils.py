...
from bot.utils import _is_in_quotes, normalize_command, split_command, validate_command
...
class TestCommandNormalization:
    ...
    def test_normalize_command_complex(self):
        ...
        result = normalize_command('find . -name "*.py" | xargs grep "import os" > results.txt && echo "Done" | cat')
        assert len(result) == 4
        assert result[0]["command"] == "find"
...
class TestCommandPermissions:
    ...
    def test_compound_commands(self):
        ...
        assert permissions.validate_command("ls -la | grep pattern | sudo cat") == CommandAction.DENY
        assert permissions.validate_command("ls -la | grep pattern | cat file.log") == CommandAction.ASK
        assert permissions.validate_command("ls -la | grep pattern | cat file.txt") == CommandAction.EXECUTE
    ...
    def test_pattern_matching(self):
        ...
        assert permissions.validate_command("curl -X POST http://api.example.com") == CommandAction.DENY
        ...
        # Edge cases
        assert permissions.validate_command("rm file.txt") == CommandAction.ASK  # Not matching rm:-rf pattern
