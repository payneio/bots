        ...
        try:
            ...
            if parsed[0] == "bash" and len(parsed) >= 3 and parsed[1] in ["-c", "-lc"]:
                ...
                try:
                    ...
                    normalized_components.append(
                        {"command": "bash", "args": parsed[1:], "has_redirection": has_redirection}
                    )
            else:
                # Default case: first word is command, rest are args
                normalized_components.append(
                    {"command": parsed[0], "args": parsed[1:], "has_redirection": has_redirection}
                )
        except Exception:
...
def validate_command(
    command: str, allow_list: List[str], deny_list: List[str], ask_if_unspecified: bool
) -> CommandAction:
